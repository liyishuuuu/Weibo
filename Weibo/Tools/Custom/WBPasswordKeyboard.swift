...
class WBPasswordKeyboard: UIInputView,
                          UITextFieldDelegate,
                          UIGestureRecognizerDelegate {
    
    // MARK: - 属性
    // 存储属性
    public static let `default` = WBPasswordKeyboard(frame: CGRect(x: 0, y: 0, width: screenWith, height: 300), inputViewStyle: .keyboard)
    ...
    ///   - target: 目标控制器
    ///   - callback: 回调
    public func customDoneButton(title: String, titleColor: UIColor = UIColor.white, theme: UIColor = UIColor.blue, target: UIViewController? = nil, callback: Selector? = nil) {
