...
class JHTextView: UITextView {
    ...
    override init(frame: CGRect, textContainer: NSTextContainer?) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(textDidChange), name: .textDidChange, object: self)
    }
...
extension JHTextView {
    ...
    func drawMaxCountText(_ rect: CGRect)  {
        ...
        let limitRect = limitText.boundingRect(with: size,
                                               options:.usesLineFragmentOrigin,
                                               attributes: [NSAttributedString.Key.font : limitFont],
                                               context:nil)
        let width:CGFloat = limitRect.size.width + 10
        let height:CGFloat = 30
        let tempRect = CGRect.init(x: rect.size.width - width,
